...
class JsonLdStrategy(ScraperStrategy):
    ...
    def extract_dealers(self, html: str, page_url: str) -> List[Dict[str, Any]]:
        ...
        print(
            f"DEBUG: JSON-LD strategy extracted {len(dealers)} dealers", file=sys.stderr
        )
        return dealers
    ...
    def _extract_dealer_from_item(
        self, item: Dict[str, Any], page_url: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"DEBUG: Error extracting dealer from JSON-LD item: {e}",
                file=sys.stderr,
            )
            return None
