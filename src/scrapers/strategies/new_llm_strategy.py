...
class NewLLMExtractorStrategy(ScraperStrategy):
    ...
    def _extract_with_llm(self, html: str, page_url: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            line = line.strip()
            if line and any(
                keyword.lower() in line.lower() for keyword in dealer_keywords
            ):
        ...
        if dealer_related_lines and len("\n".join(dealer_related_lines)) > 500:
            text_html = "\n".join(dealer_related_lines)[
                :8000
            ]  # Much larger for Ray Skillman
            print(
                f"NEW LLM DEBUG: Using {len(text_html)} RELEVANT characters (found {len(dealer_related_lines)} relevant lines)"
            )
        else:
            ...
            print(
                f"NEW LLM DEBUG: Using {len(text_html)} MIDDLE characters (no relevant content found)"
            )

        print(
            f"NEW LLM DEBUG: Using {len(text_html)} characters (reduced from {len(html)})"
        )
            ...
            try:
                ...
                for dealer in dealers:
                    if isinstance(dealer, dict) and dealer.get("name"):
                        ...
                        if processed_dealer["name"] and (
                            processed_dealer["street"] or processed_dealer["city"]
                        ):
