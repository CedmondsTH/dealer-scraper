...
class AutoCanadaStrategy(ScraperStrategy):
    ...
    def extract_dealers(self, html: str, page_url: str) -> List[Dict[str, Any]]:
        ...
        print(
            f"DEBUG: AutoCanada strategy extracted {len(dealers)} dealers",
            file=sys.stderr,
        )
        return dealers

    def _extract_dealer_from_card(
        self, card, page_url: str, canadian_provinces: set
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"DEBUG: Error extracting dealer from AutoCanada card: {e}",
                file=sys.stderr,
            )
            return None
