...
class HudsonAutomotiveStrategy(ScraperStrategy):
    ...
    def extract_dealers(self, html: str, page_url: str) -> List[Dict[str, Any]]:
        ...
        dealer_cards = soup.find_all('div', class_=lambda x: x and 'cursor-pointer' in str(x)) if soup.find_all('div', class_=lambda x: x and 'cursor-pointer' in str(x)) else []
        ...
        if not dealers:
            ...
            while i < len(lines) - 1:
                ...
                dealership_keywords = ['toyota', 'ford', 'honda', 'hyundai', 'chrysler', 
                                     'dodge', 'jeep', 'ram', 'chevrolet', 'gmc', 'buick',
                                     'nissan', 'mazda', 'volkswagen', 'subaru', 'kia']
                
                is_dealership = any(keyword in current_line.lower() for keyword in dealership_keywords)
