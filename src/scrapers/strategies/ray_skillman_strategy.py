...
class RaySkillmanStrategy(BaseScraper):
    ...
    def extract_dealers(self, html: str, url: str) -> List[Dict[str, Any]]:
        ...
        location_containers = soup.find_all(
            ["div", "section"], class_=re.compile(r"location|dealer|store", re.I)
        )
    ...
    def _extract_dealer_from_container(
        ...
        try:
            ...
            name = (
                name_elem.get_text(strip=True) if name_elem else "Ray Skillman Location"
            )
            ...
            address_pattern = re.compile(
                r"(\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Boulevard|Blvd|Road|Rd|Drive|Dr))",
                re.I,
            )
