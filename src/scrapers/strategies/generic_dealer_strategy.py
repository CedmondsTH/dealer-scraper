...
class GenericDealerStrategy(ScraperStrategy):
    ...
    def can_handle(self, html: str, page_url: str) -> bool:
        ...
        dealer_panels = soup.find_all(
            "div", class_=lambda x: x and "panel" in x and "panel-default" in x
        )
        logger.debug(
            f"Found {len(dealer_panels)} dealer panels with panel/panel-default classes"
        )
        if len(dealer_panels) >= 3:
            logger.debug(
                f"Banister panel detection SUCCESS - found {len(dealer_panels)} panels"
            )
            return True

        # Check for Bakhtiari-style dealer location cards (location class layout)
        dealer_locations = soup.find_all(
            "div", class_=lambda x: x and "location" in x and "bg-main" in x
        )
        logger.debug(
            f"Found {len(dealer_locations)} dealer locations with location/bg-main classes"
        )
        if len(dealer_locations) >= 3:
            logger.debug(
                f"Bakhtiari location detection SUCCESS - found {len(dealer_locations)} locations"
            )
            return True
        ...
        logger.debug(f"Found {colonial_count} Colonial-style dealer names")
        if colonial_count >= 3:
            logger.debug(f"Colonial detection SUCCESS - found {colonial_count} dealers")
            return True
        ...
        if "dealeron" in page_text:
            return True
        if page_url and any(
            host in page_url.lower()
            for host in [
                "banistercars.com",
                "bakhtiariauto.com",
                "aschenbachautogroup.com",
                "bakerautogroup.com",
            ]
        ):
            return True
    ...
    def _extract_banister_style_dealers(
        ...
        logger.debug(
            f"_extract_banister_style_dealers found {len(dealer_panels)} panels"
        )
        ...
        for panel in dealer_panels:
            ...
            for p in p_elements:
                ...
                if not phone_match and any(
                    keyword in p_text.lower() for keyword in address_keywords
                ):
                    lines = [
                        line.strip() for line in p_text.split("\n") if line.strip()
                    ]
                    ...
                    if len(lines) >= 2:
                        ...
                        city_line = lines[1]
                        city_match = re.match(
                            r"([^,]+),?\s*([A-Z]{2})\s*(\d{5})", city_line
                        )
                        if city_match:
                            city, state, zip_code = city_match.groups()
                            logger.debug(
                                f"Parsed address - street='{street}', city='{city}', state='{state}'"
                            )

            # If we got basic info, add the dealer
            logger.debug(
                f"Panel processed - name='{name}', street='{street}', city='{city}'"
            )
            if name and street:
                ...
                logger.debug(f"Skipping panel - missing name or street")

        return dealers

    def _extract_dealeron_locations(
        self, soup: BeautifulSoup, page_url: str
    ) -> List[Dict[str, Any]]:
        ...
        for link in soup.find_all("a"):
            ...
            if text in {
                ...
                while (
                    parent
                    and depth < 4
                    and parent.name not in ("section", "article", "li", "div")
                ):
        ...
        for container in list(containers):
            ...
            name_el = (
                ...
                or container.find_previous_sibling(["h2", "h3", "h4"])
                or container.find_previous(["h2", "h3"])
            )
    ...
    def _extract_heading_address_blocks(
        self, soup: BeautifulSoup, page_url: str
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_hgreg_dealers(
        ...
        for card in soup.select("div.car-details"):
            ...
            website = (
                name_el["href"] if name_el and name_el.has_attr("href") else page_url
            )
    ...
    def _extract_ken_ganley_dealers(
        self, soup: BeautifulSoup, page_url: str
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_group1_subpage_dealers(
        self, soup: BeautifulSoup, page_url: str
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_sierra_auto_dealers(
        self, soup: BeautifulSoup, page_url: str
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_gregory_auto_dealers(
        self, soup: BeautifulSoup, page_url: str
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_open_road_dealers(
        self, soup: BeautifulSoup, page_url: str
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_all_american_dealers(
        self, soup: BeautifulSoup, page_url: str
    ) -> List[Dict[str, Any]]:
        ...
        for h3 in soup.select("h3.h4"):
            ...
            if p_element:
                ...
                if len(address_lines) >= 2:
                    ...
                    match = re.match(
                        r"^(.+?),\s*([A-Z]{2}),?\s*(\d{5})$", city_state_zip.strip()
                    )
                    if match:
    ...
    def _extract_autobell_dealers(
        ...
        for h2 in soup.find_all("h2"):
            ...
            if address_el:
                ...
                if len(address_lines) == 2:
                    city_state_zip = BeautifulSoup(
                        address_lines[1], "html.parser"
                    ).get_text(" ", strip=True)
    ...
    def _extract_bakhtiari_style_dealers(
        self, soup: BeautifulSoup, page_url: str
    ) -> List[Dict[str, Any]]:
        ...
        dealer_cards = soup.find_all(
            "div", class_=lambda x: x and "location" in x and "bg-main" in x
        )
        logger.debug(
            f"_extract_bakhtiari_style_dealers found {len(dealer_cards)} location cards"
        )
        ...
        for card in dealer_cards:
            ...
            logger.debug(f"Bakhtiari card found {len(p_elements)} p.larger elements")

            for p in p_elements:
                p_text = p.get_text("\n", strip=True)
                logger.debug(f"Bakhtiari p_text: '{p_text}'")

                # Check if this paragraph contains a phone number
                phone_match = re.search(r"\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}", p_text)
                if phone_match:
                    phone = phone_match.group(0)
                    logger.debug(f"Bakhtiari found phone: {phone}")
                    continue
                ...
                if not phone_match and any(
                    ...
                    logger.debug(
                        f"Bakhtiari address paragraph has {len(lines)} lines: {lines}"
                    )
                    ...
                    if len(lines) >= 2:
                        ...
                        if city_match:
                            city, state, zip_code = city_match.groups()
                            logger.debug(
                                f"Bakhtiari parsed address - street='{street}', city='{city}', state='{state}'"
                            )

            # If we got basic info, add the dealer
            logger.debug(
                f"Bakhtiari processed - name='{name}', street='{street}', city='{city}'"
            )
            if name and street:
                logger.debug(f"Adding Bakhtiari dealer: {name}")
                dealers.append(
                    {
                        "name": name,
                        "street": street,
                ...
                logger.debug(f"Skipping Bakhtiari card - missing name or street")

        return dealers

    def _extract_colonial_style_dealers(
        self, soup: BeautifulSoup, page_url: str
    ) -> List[Dict[str, Any]]:
        ...
        logger.debug(
            f"_extract_colonial_style_dealers found {len(colonial_divs)} dealer divs"
        )

        for div in colonial_divs:
            name = div.get_text().strip()
            logger.debug(f"Processing Colonial dealer: {name}")
            ...
            while current and siblings_checked < 10:  # Look at next few siblings
                if hasattr(current, "get_text"):
                    ...
                    logger.debug(f"Colonial sibling text: '{text}'")
                    ...
                    if text:  # Non-empty text
                        ...
                        if phone_match and not phone:
                            phone = phone_match.group(0)
                            logger.debug(f"Found Colonial phone: {phone}")
                        ...
                        # Check for address pattern (street + city, state zip)
                            ...
                            if len(parts) >= 2:
                                ...
                                city_match = re.match(
                                    r"^([^,]+?)\s+([A-Z]{2})\s+(\d{5})$", remaining
                                )
                                if city_match:
                                    street = potential_street
                                    city, state, zip_code = city_match.groups()
                                    logger.debug(
                                        f"Found Colonial address - street='{street}', city='{city}', state='{state}', zip='{zip_code}'"
                                    )
            ...
            if name and street and city and state:
                ...
                logger.debug(f"Added Colonial dealer: {name} at {street}")
            else:
                logger.debug(
                    f"Skipping Colonial dealer - missing info: name={bool(name)}, street={bool(street)}, city={bool(city)}, state={bool(state)}"
                )
