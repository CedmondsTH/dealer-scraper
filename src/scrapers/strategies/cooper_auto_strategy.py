...
class CooperAutoStrategy(ScraperStrategy):
    ...
    def extract_dealers(self, html: str, page_url: str) -> List[Dict[str, Any]]:
        ...
        if extracted_dealers:
            print(
                f"DEBUG: Successfully extracted {len(extracted_dealers)} dealers from HTML",
                file=sys.stderr,
            )
        ...
        print(
            f"DEBUG: Cooper Auto strategy returning {len(processed_dealers)} dealers",
            file=sys.stderr,
        )
    ...
    def _extract_from_html(
        ...
        if phone_matches:
            print(
                f"DEBUG: Found {len(phone_matches)} phone numbers: {phone_matches}",
                file=sys.stderr,
            )

        # Look for address patterns
        address_pattern = re.compile(
            r"(\d+.*?(?:Ave|Avenue|St|Street|Blvd|Boulevard|Rd|Road|Dr|Drive))",
            re.IGNORECASE,
        )
        ...
        if address_matches:
            print(
                f"DEBUG: Found {len(address_matches)} addresses: {address_matches}",
                file=sys.stderr,
            )
        ...
        for heading in headings:
            ...
            if any(
                brand in heading_text.lower()
                for brand in ["cooper", "chevrolet", "gmc", "hyundai"]
            ):
