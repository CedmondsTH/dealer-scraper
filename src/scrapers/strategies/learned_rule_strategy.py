...
class LearnedRuleExtractorStrategy(ScraperStrategy):
    ...
    def extract_dealers(self, html: str, page_url: str) -> List[Dict[str, Any]]:
        ...
        for r in rules:
            ...
            for card in cards:
                name_el = (
                    card.select_one(r.fields.get("name", ""))
                    if r.fields.get("name")
                    else None
                )
                name = name_el.get_text(strip=True) if name_el else ""
                street_el = (
                    card.select_one(r.fields.get("street", ""))
                    if r.fields.get("street")
                    else None
                )
                street = street_el.get_text(strip=True) if street_el else ""
                csz_el = (
                    card.select_one(r.fields.get("city_state_zip", ""))
                    if r.fields.get("city_state_zip")
                    else None
                )
                ...
                phone = ""
                ph_el = (
                    card.select_one(r.fields.get("phone", ""))
                ...
                a = (
                    card.select_one(website_selector)
                    if website_selector
