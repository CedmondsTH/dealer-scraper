...
class AddressParser:
    ...
    def parse_address(self, address_text: str) -> Tuple[str, str, str, str]:
        ...
        # Normalize pipe separators to commas
    ...
    def _format_canadian_postal_code(self, postal_code: str) -> str:
        ...
        if (
            re.match(self.canadian_postal_pattern, postal_code)
            and " " not in postal_code
        ):
    ...
    def _parse_fallback(self, address_text: str) -> Tuple[str, str, str, str]:
        ...
        # If we have only 2 parts, likely "City, State ZIP" (no street)
        if len(parts) == 2:
            street = ""
            city = parts[0]
            state, postal_code = self._extract_state_zip_regex(parts[1])
            return street, city, state, postal_code
