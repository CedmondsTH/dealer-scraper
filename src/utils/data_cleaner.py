...
class DataCleaner:
    ...
    def is_valid_dealership(self, dealer_data: Dict[str, Any]) -> bool:
        ...
        name = (
            (dealer_data.get("name", "") or dealer_data.get("Name", "") or "")
            .strip()
            .lower()
        )
        street = (
            dealer_data.get("street", "") or dealer_data.get("Street", "") or ""
        ).strip()
        website = (
            dealer_data.get("website", "") or dealer_data.get("Website", "") or ""
        ).strip()
        ...
        if len(name) > 80 or any(
            ...
            logger.debug(
                f"Invalid dealer: name '{name}' appears to be descriptive text"
            )
            return False
    ...
    def extract_car_brands(self, name: str) -> str:
        ...
        if all(
            word in name_lower for word in ["chrysler", "jeep", "dodge", "ram", "fiat"]
        ):
            return "CDJRF"
        ...
        found_brands = [
            brand for brand in self.car_brands if brand.lower() in name_lower
        ]
    ...
    def determine_country(self, state_province: str) -> str:
        ...
        return (
            "Canada"
            if state_province.strip().upper() in self.canadian_provinces
